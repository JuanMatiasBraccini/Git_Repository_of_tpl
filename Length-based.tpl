...
DATA_SECTION
  ...
  init_int Calc_MSY
  init_int yrs_MSY
  init_vector Fish_mort(1,yrs_MSY)
  init_vector Rec_error_MSY(1,yrs_MSY)    
...
PARAMETER_SECTION
  ...
  // matrix depletion_spawn(1,1,1,nzone)
   ...
   //Declare objects used in MSY()
   4darray Num_in_len_class_MSY(1,N_sex,1,yrs_MSY+1,1,nzone,1,N_inc)
   3darray Total_biom_sex_MSY(1,N_sex,1,yrs_MSY,1,nzone)
   matrix Fem_spawn_biom_MSY(1,yrs_MSY,1,nzone)
   3darray Annual_F_sex_MSY(1,N_sex,1,yrs_MSY,1,nzone)
   4darray Surv_in_len_class_MSY(1,N_sex,1,yrs_MSY,1,nzone,1,N_inc)
   4darray Est_catch_in_len_class_MSY(1,N_sex,1,yrs_MSY,1,nzone,1,N_inc)  
   3darray Est_catch_MSY(1,N_sex,1,yrs_MSY,1,nzone)
   matrix Annual_rec_MSY(2,yrs_MSY+1,1,nzone)
   4darray Num_in_len_class_temp_MSY(1,N_sex,1,yrs_MSY+1,1,nzone,1,N_inc)
   matrix Total_biom_MSY(1,yrs_MSY,1,nzone)
   matrix Total_ktch_MSY(1,yrs_MSY,1,nzone)
   matrix Total_Fem_spawn_biom_MSY(1,yrs_MSY,1,nzone)
...
FUNCTION Move_rate
  ...
  alpha22=exp(p22)/(1+exp(p22)+exp(p21));
  alpha21=exp(p21)/(1+exp(p22)+exp(p21));
  alpha33=exp(p33)/(1+exp(p33));
  
   //2. fill in daily Movement matrix
  Mov_mat(1,1)=alpha11;
  Mov_mat(1,2)=1-alpha11;
  Mov_mat(1,3)=0;

  Mov_mat(2,1)=alpha21;
  Mov_mat(2,2)=alpha22;
  Mov_mat(2,3)=1-(alpha22+alpha21);

  Mov_mat(3,1)=0;
  Mov_mat(3,2)=1-alpha33;
  Mov_mat(3,3)=alpha33;
  ...
  //3. Predict location of recaptured shark

    //3.1 Conventional tags 
  for(t=1; t<=nrec_c;t++)
  {
    Move_c=Mov_mat;
...
FUNCTION MSY
  for (s=1; s<=N_sex;s++)                         //loop over each sex
  {
   ...
   for(z=1; z<=nzone;z++)                        //loop over zone
   {
     for (i=1; i<=N_inc;i++)                     //loop over size bins
     {
       Num_in_len_class_MSY(s,1,z,i) = Num_in_len_class(s,Yrs_future,z,i);  
     } //i  
   }//z
   
   // future calculations
   for (t=1; t<=yrs_MSY;t++)                    //loop over time 
   {
     for(z=1; z<=nzone;z++)                        //loop over zone
     {
      //1. calculate biomasses @ time t
      ...
      {
        //total biomass (already in tonnes)
        Total_biom_sex_MSY(s,t,z)+=Num_in_len_class_MSY(s,t,z,i)*TwT(s,i);     
        
        //spawning biomass (already in tonnes)
        if(s==1) Fem_spawn_biom_MSY(t,z)+=Num_in_len_class_MSY(s,t,z,i)*TwT(s,i)*Mat(i);          
      } //i
      ...
      //2.1 get fully selected annual F
       Annual_F_sex_MSY(s,t,z)=Fish_mort(t);     
      
      //2.2 apply M and F by size to calculate population numbers    
      for (i=1; i<=N_inc;i++)
      {
        // calculate the length-specific fishing mortality accounting for selectivity
        F_in_len_class = SEL_zone_yr(z,i,Yrs_future) * Annual_F_sex_MSY(s,t,z);
        
        // calculate the total mortality
        Z_in_len_class = F_in_len_class + M_sex(s,i);
         
        // calculate predicted numbers per size class that will survive to the beginning of next year
        Surv_in_len_class_MSY(s,t,z,i) = Num_in_len_class_MSY(s,t,z,i) * mfexp(-Z_in_len_class);        
        
        // calculate predicted catch in weight (tonnes)
        Est_catch_in_len_class_MSY(s,t,z,i) = TwT(s,i)* Num_in_len_class_MSY(s,t,z,i)*(F_in_len_class/Z_in_len_class)*(1-mfexp(-Z_in_len_class));     
        
        // sum predicted catch (tonnes) over all length classes
        Est_catch_MSY(s,t,z) += Est_catch_in_len_class_MSY(s,t,z,i);
        
        // grow  animals and place them into the next time step        
        for (j=1; j<=N_inc;j++)
        {
          Num_in_len_class_MSY(s,t+1,z,j) += Surv_in_len_class_MSY(s,t,z,i) * STM(s,j,i);      
        } // j
    ...
    if(s==1)
    {
      Rec_temp=sum(Fem_spawn_biom_MSY(t)) / (aSRR + (bSRR * sum(Fem_spawn_biom_MSY(t))));
      for(z=1; z<=nzone;z++)
      {
        Annual_rec_MSY(t+1,z) = Rec_temp*R_zero_prop(z)*Rec_error_MSY(t);  //add assumed rec variability   
        if(Annual_rec_MSY(t+1,z)> Virgin_Recruitment(z)) Annual_rec_MSY(t+1,z)=Virgin_Recruitment(z);          
      }        
    }

    //4. Add expected recruits to the population, for next year
    for(z=1; z<=nzone;z++) for (i=1; i<=N_inc;i++)  Num_in_len_class_MSY(s,t+1,z,i) += Prob_at_len_rec(i) * Annual_rec_MSY(t+1,z)*Sex_ratio;
    ...
    if(Do_move==1)
    {
      ...
      // first calculate numbers after movement for all size classes
      for(from=1;from<=nzone;from++) for(to=1;to<=nzone;to++) Num_in_len_class_temp_MSY(s,t,to)+=Num_in_len_class_MSY(s,t,from)* Mov_mat_annual(from,to) ;   
      
      // second replace numbers for size assumed to be able to move
      for(to=1;to<=nzone;to++) Num_in_len_class_MSY(s,t,to)(move_min_size,N_inc)=Num_in_len_class_temp_MSY(s,t,to)(move_min_size,N_inc);
    }
  ...
  //6. Sum female and male total biomass and catch
  for(z=1; z<=nzone;z++)
  {
   Total_biom_MSY.colfill(z,column(Total_biom_sex_MSY(1),z)+column(Total_biom_sex_MSY(2),z));
   Total_ktch_MSY.colfill(z,column(Est_catch_MSY(1),z)+column(Est_catch_MSY(2),z));
   Total_Fem_spawn_biom_MSY.colfill(z,column(Fem_spawn_biom_MSY,z));
  }

  cout<<"Total_ktch_MSY\n"<<Total_ktch_MSY<<endl;

  cout<<"Virgin_Total_biom\n"<<Virgin_Total_biom<<endl;
  cout<<"Total_biom_MSY\n"<<Total_biom_MSY<<endl;

  cout<<"Virgin_Spawn_Biom\n"<<Virgin_Spawn_Biom<<endl;
  cout<<"Female_Spawning_biom_MSY\n"<<Total_Fem_spawn_biom_MSY<<endl;
  exit(1);
